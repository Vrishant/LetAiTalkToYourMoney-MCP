...
//   }
// );
server.tool("take-financial-data", "Load user's loan and income data from the local assets folder.", {}, async () => {
    try {
...
server.tool("optimize-debt-plan", "Optimize debt using snowball or avalanche method.", {
    ...
    const loans = [...data];
    loans.sort((a, b) => strategy === "avalanche" ? b.interestRate - a.interestRate : a.principal - b.principal);
...
server.tool("get-explainer", "Returns a Gemini-style explanation for the recommended strategy.", {
    strategy: z.enum(["snowball", "avalanche"]),
    interestSaved: z.number().describe("Amount of interest saved by using this strategy"),
}, async ({ strategy, interestSaved }) => {
    const message = strategy === "avalanche"
...
server.tool("warn-leverage-risk", "Warn the user if their EMI exceeds a healthy % of their income.", {
    ...
    if (ratio > 50)
        status = "⚠️ Critical: Your EMI exceeds 50% of your income.";
    else if (ratio > 35)
        status = "⚠️ Caution: EMI is over 35%, consider revising.";
    return {
        content: [
...
server.tool("export-plan", "Export the user's plan to a file format (CSV/PDF).", {
    ...
    try {
        const content = format === "csv"
            ? planData.map((e) => `${e.month},${e.emi}`).join("\n")
